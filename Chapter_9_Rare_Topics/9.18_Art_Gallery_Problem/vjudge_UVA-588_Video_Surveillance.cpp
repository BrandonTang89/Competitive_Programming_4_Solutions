/**vjudge - UVA-588 - Video Surveillance
 * Determine if an art gallery is defensable by 1 guard. We repeatedly cut away spaces in the
 * polygon where the guard cannot stand, then we see if we have an empty polygon at the end. 
 * 
 * Time: O(n), Space: O(n)
 */
...
int main() {
    ...
    for (int floor=1; ;floor++){
        ...
        cout << ((int) space.size() != 0 ? "Surveillance is possible." : "Surveillance is impossible.") << endl;
        cout << endl;
    }
    return 0;
}