/**Kattis - shopping
 * Let S be the set of {shops to visit} union {0}. We want to do TSP on S starting from 0 and ending
 * back at 0. To do this TSP, we need to Dijkstra from each u in S and update an adjacency matrix.
 * Relatively routine question.
 *
 * Time: O(E log E + 2^s * s^2), Space: O(E + V + 2^s * s)
 */
